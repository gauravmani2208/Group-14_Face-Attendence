...
def add_attendance(name):
    ...
    # Check if the user's attendance has been marked within the last hour
    one_hour_ago = (current_time - timedelta(hours=1)).time()

    # Filter by user and check if there is any attendance marked within the last hour
    recent_attendance = df[(df['Roll'] == str(userid)) & (df['Time'] >= one_hour_ago)]
    
    print("roll === userid",(df['Roll'] == str(userid)),"time === oha",(df['Time'] >= one_hour_ago) )


    if not recent_attendance.empty:
        print(f"Attendance for {username} ({userid}) has already been marked within the last hour.")
        return
    ...
    print(f"Attendance marked for {username} ({userid}) at {current_time_str}")
